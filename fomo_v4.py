...
with tab1:
    ...
    if not results:
        ...
        st.markdown('<div style="height:10px;"></div>',unsafe_allow_html=True)
        st.markdown('<div style="font-size:.58rem;font-weight:700;color:#3D5A7A;text-transform:uppercase;letter-spacing:.15em;margin:14px 0 8px 0;padding-bottom:5px;border-bottom:1px solid #0C1F35;">Predictive Signal — What History Says Happens Next</div>',unsafe_allow_html=True)
        same_bias = [e for e in EVENTS if e["bias"] == ev["bias"]]
        reverted = [e for e in same_bias if e.get("rev")]
        rev_prob = int(len(reverted)/len(same_bias)*100) if same_bias else 0
        if len(same_bias) >= 15:
            conf_level = "HIGH"
            conf_color = "#34D399"
            conf_desc = str(len(same_bias)) + " historical matches — statistically meaningful"
        elif len(same_bias) >= 8:
            conf_level = "MEDIUM"
            conf_color = "#F59E0B"
            conf_desc = str(len(same_bias)) + " historical matches — moderate confidence"
        else:
            conf_level = "LOW"
            conf_color = "#F87171"
            conf_desc = str(len(same_bias)) + " historical matches — interpret with caution"
        st.markdown(f'<div style="background:#060D18;border:1px solid '+conf_color+';border-radius:10px;padding:12px 18px;margin-bottom:12px;"><span style="font-size:.62rem;font-weight:700;color:#3D5A7A;text-transform:uppercase;letter-spacing:.1em;">Confidence Level</span> <span style="font-size:.85rem;font-weight:800;color:'+conf_color+';">'+conf_level+'</span> <span style="font-size:.72rem;color:#64748B;">— '+conf_desc+'</span></div>',unsafe_allow_html=True)
        avg_rev_days = int(sum(e["rev"] for e in reverted)/len(reverted)) if reverted else 0
        avg_peak_similar = sum(abs(e["peak"]) for e in same_bias)/len(same_bias) if same_bias else 0
        reverted_names = ", ".join([e["event"] for e in reverted])
        not_reverted_names = ", ".join([e["event"] for e in same_bias if not e.get("rev")]) or "None"
        if ev["score"] >= 80:
            sig_color = "#EF4444"
            sig_txt = "STRONG REVERSION SIGNAL"
            act_txt = str(len(same_bias)) + ' historical ' + ev['bias'] + ' events — ' + str(rev_prob) + '% showed clear mean reversion within ' + str(avg_rev_days) + ' trading days on average.'
        elif ev["score"] >= 65:
            sig_color = "#F59E0B"
            sig_txt = "MODERATE REVERSION SIGNAL"
            act_txt = str(len(same_bias)) + ' historical ' + ev['bias'] + ' events — ' + str(rev_prob) + '% reverted within ' + str(avg_rev_days) + ' days on average.'
        else:
            sig_color = "#10B981"
            sig_txt = "LOW REVERSION SIGNAL"
            act_txt = 'Market absorbed this event relatively rationally. ' + str(rev_prob) + '% of similar events showed reversion.'
        w1,w2,w3,w4=st.columns(4)
        with w1:
            st.markdown(f'''<div class="kpi"><div class="kpi-val" style="color:#F87171;">{rev_prob}%</div><div class="kpi-lbl">Reversion probability</div></div>''',unsafe_allow_html=True)
        with w2:
            st.markdown(f'''<div class="kpi"><div class="kpi-val" style="color:#34D399;">{avg_rev_days}d</div><div class="kpi-lbl">Avg days to reversion</div></div>''',unsafe_allow_html=True)
        with w3:
            st.markdown(f'''<div class="kpi"><div class="kpi-val" style="color:#FCD34D;">{len(same_bias)}</div><div class="kpi-lbl">Similar events in database</div></div>''',unsafe_allow_html=True)
        with w4:
            st.markdown(f'''<div class="kpi"><div class="kpi-val" style="color:#A78BFA;">{avg_peak_similar:.0f}%</div><div class="kpi-lbl">Avg peak CAR similar</div></div>''',unsafe_allow_html=True)
        proof = [
            ("Reversion probability", str(len(reverted)) + ' out of ' + str(len(same_bias)) + ' ' + ev['bias'] + ' events confirmed reversion. ' + str(len(reverted)) + '/' + str(len(same_bias)) + ' = ' + str(rev_prob) + '%.'),
            ("Events that reverted", reverted_names),
            ("Events unresolved", not_reverted_names),
            ("Academic backing", "DeBondt and Thaler (1985) Does the Stock Market Overreact — Journal of Finance. Foundational paper on mean reversion after behavioral events."),
            ("Selection criteria", "Events selected where S&P 500 or TSX experienced abnormal returns exceeding 3% within 5 trading days cross-referenced with major financial news coverage."),
            ("Honest limitation", f"Database contains {len(EVENTS)} events. Larger sample improves statistical confidence. Past patterns do not guarantee future results. Not financial advice."),
        ]
        ph = '<div class="panel">'
        for k,v in proof:
            ph += f'<div class="row"><span class="rk">{k}</span><span class="rv">{v}</span></div>'
        ph += '</div>'
